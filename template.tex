...
\Title{A lost circulation monitoring model using multi-scale feature fusion based on wavelet transform and TimeGAN}

% MDPI internal command: Title for citation in the left column
\TitleCitation{A lost circulation monitoring model using multi-scale feature fusion based on wavelet transform and TimeGAN}
...
\abstract{The issue of lost circulation represents a significant challenge in drilling, given its potential to impede both safety and efficiency. The traditional monitoring model is hindered by the presence of noise and the complexity of temporal fluctuations in lost circulation data, resulting in suboptimal performance with regard to accuracy and generalisation ability. This, in turn, complicates the adaptation of the model to diverse working conditions.To address these limitations, this paper proposes a multi-scale feature fusion model based on wavelet transform and TimeGAN. The model employs wavelet transform to enhance time-series features, TimeGAN to expand scarce lost circulation data, convolutional network feature extraction, and feature fusion through a multi-scale feature fusion module to mine the correlation between different features and achieve time-series information capture. The experimental findings demonstrate that the multi-scale feature fusion model proposed in this paper enhances the accuracy by 8.8\% , reduces the leakage rate and false alarm rate by 12.4\%  and 6.2\% , respectively, and attains a test set accuracy of 93.8\%  and an accuracy of 95.1\%  in the lost circulation identification task in comparison to the unoptimized model. The method outlined in this paper provides reliable technical support for the monitoring of lost circulation risk, thereby contributing to the enhancement of safety and efficiency in the drilling process.}
